...
var SuggestList = React.createClass({displayName: 'SuggestList',
	getSuggest: function(value){
		...
		$.ajax({
			...
			success: function(data){
				if(data.length){
					this.setState({show: true,lists: this.renderList(data)})
				}else{
					this.setState({show: false,lists: []})
				}
	...
	render: function(){
		...
		if(!this.isMounted() || !this.props.value){
			this.state.lists = []
			show = false
		}else{
		...
		return (
			React.DOM.ul({className: "suggestList", style: {display: show ? 'block' : 'none'}}, 
				this.state.lists
			)
		)
	}
...
var SearchBar = React.createClass({displayName: 'SearchBar',
	...
	suggestHandler: function(e){
		this.props.suggestHandler(e.target.value)
	},
	...
	render: function(){
		...
		React.DOM.div({className: "row collapse"}, 
			...
			SuggestList({value: this.props.suggestList, onClickHandler: this.props.onClickHandler, resetHandler: this.resetHandler})
			), 
...
var App = React.createClass({displayName: 'App',
	...
	render: function(){
		...
		return (
			React.DOM.div(null, 
				SearchBar({onClickHandler: this.onClickHandler, suggestHandler: this.suggestHandler}), 
